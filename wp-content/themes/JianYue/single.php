		...
		<main id="main" class="site-main" role="main">
			...
			<?php if(get_post_meta($post->ID, 'wzzz', true)){$wzzz = get_post_meta($post->ID, 'wzzz', true);}else{$wzzz=get_the_author();}?>
				<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
					...
					<?php if ( get_post_meta($post->ID, 'tgwz', true) ) { ?>投稿文章<?php } elseif ( get_post_meta($post->ID, 'zzwz', true) ) { ?>转载文章<?php }else{?>原创文章<?} ?>&nbsp;·&nbsp;<?php }if( function_exists( 'the_views' ) ) { the_views(); print '阅读'; } ;?>&nbsp;·&nbsp;<?php comments_popup_link( '0评论', '1评论', '%评论' ); ?>
						</div>		
					</header><!-- .entry-header -->
